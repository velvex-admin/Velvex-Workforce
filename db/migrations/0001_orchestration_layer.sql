...
crg4te or replace function public.memory_touch_updated_at()
...
create index if not exists pending_approvals_agent_idx  on public.pending_approvals (agent_id);

-- ----------------------------------------------------------------------------
-- Lock the tables down. The Worker talks to Supabase with the service-role key,
-- which bypasses RLS. Enabling RLS with no policies means the application
